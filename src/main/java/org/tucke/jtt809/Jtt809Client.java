...
public class Jtt809Client {
    ...
    public static void newClient(Integer gnsscenterId, UpConnectPacket.Request request, OuterPacket activePacket) {
        ...
        NettyClient client = new NettyClient(request.getDownLinkIp(), request.getDownLinkPort(), new ChannelInitializer<>() {
            ...
            protected void initChannel(Channel ch) throws Exception {
                ...
                ch.pipeline().addLast(new Jtt809SlaveInboundHandler(gnsscenterId, request, activePacket));
            }
        });
        try {
            client.connect();
    ...
    public static void reconnect(Integer gnsscenterId, UpConnectPacket.Request request, OuterPacket activePacket) {
        int retry = 0;
        ...
        if (retry < 3) {
            ...
            // 每 5 秒进行一次重连
            eventLoop.schedule(() -> Jtt809Client.reconnect(gnsscenterId, request, activePacket), 5, TimeUnit.SECONDS);
            newClient(gnsscenterId, request, activePacket);
        } else {
            close(gnsscenterId);
